...
@section('content')
	@if ($medidas != '[]')
	    ...
	    <h3 class="text-uppercase mt-40">LISTADO DE TERMOGRAFÍAS - {{ $sucursales->name }}</h3>
		            	...
		            	@foreach($medidas as $m)
		            		@if($sucursales->id === $m->idbranch_office)
				            	<tr>
				            		...
				            		<td class="text-center">
				            			<a href="{!! route('sucursal.show', [$sucursales->id,'termografia', $m->id]); !!}" class="fa-link-tmm text-merge">ver detalle</a>
				            		</td>
				            	</tr>
		            		@endif
		            	@endforeach
