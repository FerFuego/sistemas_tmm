...
@section('title-section','LISTADO DE MEDICIONES DE DIFERENCIALES')

@section('breadcrumbs')
  <ol class="breadcrumb">
    <li><a href="{{ url('usuarios/todos') }}">usuarios</a></li>
    <li><a href="{{ url('usuarios/'.$user->id.'/ver') }}">{{ $user->name}}</a></li>
    <li  class="active">Diferenciales</li>
  </ol>
@endsection
...
@section('content')
  @if ($medidas != '[]')
        ...
        <div class="col-md-6">
           @if ($alarma != '[]')
              @php $type = false; @endphp
                  @foreach($alarma as $a)
                        @if($a->type == 'diferencial' || $a->type == 'diferenciles')
                          @php 
                            $count = $a->count; 
                            $type = true;
                          @endphp 
      ...
      <h3 class="text-uppercase">LISTADO DE DIFERENCIALES</h3>
        ...
        <div class="row">
          <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            <div class="white-box">
                    ...
                    @foreach($medidas as $m)
                      @if($s->id === $m->idbranch_office)
                        <tr>
                          ...
                          <td class="text-center">
                            <a href="{!! route('usuario.sucursal.show', [$user->id, $s->id,'diferencial', $m->id]); !!}" class="fa-link-tmm text-merge"><i class="icon-eye"></i></a>
                            <a href="{!! route('measurement/duplicate', [$m->id]) !!}" class="fa-link-tmm text-info"><i class="fa fa-copy"></i></a>
              ...
              @else
                <p class="text-uppercase mt-40">NO EXISTEN MEDICIONES DE DIFERENCIALES </p>
              @endif
            </div>
          </div>
        </div>
      ...
      <!-- Fin Modal_medicion -->
    @else
      <h3 class="text-uppercase mt-40">NO EXISTEN MEDICIONES DE DIFERENCIAL </h3>
    @endif
@endsection 
